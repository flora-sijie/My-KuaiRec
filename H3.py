...
class Config:
    ...
    batch_size = 2048
    lr_worker = 0.001
    lr_manager = 0.001 # 已调小，防止 Manager 过拟合
    weight_decay = 1e-4
    
    # Worker 参数 (CreatorXSimGCL)
    emb_dim = 64
    n_layers = 3
    cl_rate = 0.2
    eps = 0.1
    ...
    manager_scale = 0.1 # [核心] 缩放因子，防止 Manager 初始权重过大
    
    # 评估参数
    top_k = 20

config = Config()
print(f"🚀 Phase 2 Full Model (H3-Score Ver.) running on {config.device}")
...
class RecDataset(Dataset):
    def __init__(self, conf):
        ...
        # 生成 Item-level 的 Tail Mask (0=Tail, 1=Mid, 2=Head)
        # 我们假设 group 0 是 tail
...
class ManagerNetwork(nn.Module):
    ...
    def forward(self, active_level_ids):
        ...
        weight = self.net(emb) # [batch, 1]
        return weight

# --- Part A: Worker (CreatorXSimGCL) ---
...
class JointModel(nn.Module):
    ...
    def predict(self, u_batch, i_batch, use_manager=True):
        ...
        # [核心] 引入 scale 防止喧宾夺主
        final_score = base_score + (boost_weight * is_tail * self.conf.manager_scale)
        return final_score
...
def evaluate_h3(model, dataset, top_k=20):
    model.eval()
    NDCG_list, FanR_list = [], []
    recommended_item_counts = {} # For Gini
    ...
    with torch.no_grad():
        ...
        for start in range(0, len(test_users), 100):
            ...
            weights = model.manager(u_active)
            is_tail = dataset.is_tail_item.unsqueeze(0)
            scores = scores + (weights * is_tail * config.manager_scale)
...
if __name__ == '__main__':
    ...
    for epoch in range(config.epochs):
        ...
        for batch_u, batch_pos_i in dataloader:
            ...
            # 3. Ecosystem Loss (Hinge)
            is_tail_pos = dataset.is_tail_item[batch_pos_i]
            # 简单激励：如果是 Tail，则希望 score_pos 越大越好 (margin=1.0)
            # 只有当 score_pos < 1.0 时才惩罚，促使 Manager 给它加分
            eco_loss = 0
    ...
    print(f"\n✅ Training Finished! Final H3: {h3:.4f}")